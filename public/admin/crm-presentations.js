...
export function mountPresentations({panel,company,api,notify}) {
  ...
  async function load(){
    ...
    try{
      ...
      panel.innerHTML=`<div class="section-title"><h2>Presentations</h2><button class="primary" data-link-deck ${company.archived_at?'disabled':''}>Assign presentation</button></div>
        ...
        <a class="quiet" href="/admin">Open Presentation Studio to create a deck</a><p role="alert" data-presentation-error></p>`+
        (items.length?items.map(item=>{
          ...
          <button class="quiet" data-unlink-deck="${esc(item.id)}" ${company.archived_at?'disabled':''}>Unassign</button></div>
  ...
  async function click(e){
    ...
    const item=items.find(i=>i.id===(b.dataset.markSent||b.dataset.unlinkDeck||b.dataset.deckAnalytics));if(!item)return;
  ...
  return ()=>{disposed=true;seq++;dialogSeq++;panel.removeEventListener('click',click);dialog.close();dialog.remove();};
}
