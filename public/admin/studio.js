...
function renderProjects() {
 ...
 $('#projects').innerHTML=filtered.length ? filtered.map(p=>{
  if(isTemplate(p)) return `<article class="project-row template-row"><button type="button" class="project-icon" data-template-preview="${p.deck_slug}" aria-label="Preview ${escape(p.client)}">${arrowIcon}</button><button class="project-name" data-template-preview="${p.deck_slug}"><strong>${escape(p.client)}</strong><small>${escape(p.title)} · Reusable template</small></button><span class="badge">Template</span><div class="row-actions"><button class="secondary" data-variant="${p.deck_slug}">Use template</button><button class="quiet" data-edit="${p.deck_slug}">Edit</button><button class="quiet delete-action" data-delete="${p.deck_slug}">Delete</button></div></article>`;
...
function openEdit(slug){const p=projects.find(p=>p.deck_slug===slug),form=$('#edit-form');form.elements.slug.value=slug;form.elements.title.value=p.title;form.elements.status.value=p.status;form.elements.isTemplate.checked=p.is_template;form.elements.match.required=false;form.elements.match.closest('label').hidden=true;$('#edit-dialog').showModal();}
...
$('#delete-form').onsubmit=e=>{e.preventDefault();busy(e.currentTarget,async()=>{const confirmSlug=e.target.elements.confirmSlug.value.trim();if(confirmSlug!==deletingSlug)throw new Error('Type the exact URL name to confirm deletion.');const result=await api('delete',{slug:deletingSlug,confirmSlug});$('#delete-dialog').close();await refresh();notify(result.sharedFiles?'Presentation deleted. Shared files remain for other presentations.':'Presentation, files and statistics deleted.');});};
