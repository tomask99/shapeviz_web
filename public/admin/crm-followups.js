...
export function createFollowups({root,api,notify}) {
  const dialog=document.createElement('dialog');dialog.className='crm-record-dialog';dialog.id='followup-dialog';dialog.setAttribute('aria-labelledby','followup-title');document.body.append(dialog);
  let active=false,sequence=0,epoch=0,groups=[],company=null,pending=false,clock,bounds,dialogSequence=0;
  const companyId=()=>new URLSearchParams(location.search).get('companyId')||'';
  const query=()=>({...bounds,...(companyId()?{companyId:companyId()}:{})});
  function render() {
    root.querySelector('#followup-groups').innerHTML=groups.map(g=>`<section class="followup-group" aria-label="${groupNames[g.key]}"><header><h2>${groupNames[g.key]}</h2><span class="badge">${g.total}</span></header>${g.items.map(f=>`<article class="crm-entry" data-followup="${esc(f.id)}">
  ...
  async function load(group=null,page=1) {
    ...
    const target=root.querySelector('#followup-groups'),error=root.querySelector('#followup-error');
    ...
    finally{if(active&&ticket===sequence)target.setAttribute('aria-busy','false');}
  }
  async function edit(record=null) {
    if(pending)return;
    const life=epoch,ticket=++dialogSequence;
    dialog.innerHTML=`<form><div class="dialog-head"><h2 id="followup-title">${record?'Reschedule':'Schedule'} follow-up.</h2><button type="button" data-cancel aria-label="Close">×</button></div>
      ${record||company?`<p class="fine">${esc(record?.company_name||company.company_name)}</p>`:`<label>Find company<input name="company_search" type="search" maxlength="160" placeholder="Search name or website"></label><button type="button" class="secondary" data-find-company>Search companies</button><label>Company *<select name="company_id" required><option value="">Choose a company…</option></select></label><p class="fine" data-company-hint></p>`}
    ...
    let selected=record?.company_id||company?.id||'',contactsPage=0,contactSeq=0,finding=0,contactsBusy=false;
    const alive=()=>active&&epoch===life&&ticket===dialogSequence&&dialog.open;
    form.elements.title.value=record?.title||'';form.elements.description.value=record?.description||'';
    ...
    form.onsubmit=async e=>{
      ...
      try{await api('crm-followup-save',data);if(!alive())return;dialog.close();notify('Follow-up saved.');await load();}
  ...
  async function complete(id) {
    ...
    try{await api('crm-followup-complete',{companyId:record.company_id,id,version:record.version});if(active&&life===epoch){notify('Follow-up completed.');await load();}}
    catch(e){if(active&&life===epoch){root.querySelector('#followup-error').textContent=e.message+' Refresh before retrying.';render();}}
  ...
  function checkDay(){if(active&&!pending&&!dialog.open&&bounds.today!==localDayBounds().today){bounds=localDayBounds();load();}}
  async function show() {
    active=true;pending=false;groups=[];company=null;const life=++epoch;bounds=localDayBounds();
    root.innerHTML=`<div class="page-heading"><div><p class="eyebrow">YOUR NEXT CONVERSATION.</p><h1>Follow-ups<span>.</span></h1></div><button class="primary" data-schedule-followup disabled>Schedule follow-up +</button></div>
      <p class="fine" id="followup-scope">Loading…</p><div class="actions"><button class="secondary" data-followup-refresh>Refresh</button><a class="quiet" href="/admin/follow-ups" data-lead>All follow-ups</a><a class="quiet" href="/admin/leads" data-lead>Open Leads</a></div>
      <p class="fine">Dates use your local timezone. Overdue means before today; archived companies are hidden.</p><p id="followup-error" role="alert"></p><div id="followup-groups"></div>`;
    try {
      if(companyId()){const data=await api('crm-detail',null,{id:companyId()});if(!active||epoch!==life)return;company=data.company;}
      root.querySelector('#followup-scope').textContent=company?`${company.company_name}${company.archived_at?' — archived. Restore this company to manage its follow-ups.':''}`:'All active companies';
      root.querySelector('[data-schedule-followup]').disabled=!!company?.archived_at;
      await load();
    }catch(e){if(active&&epoch===life)root.querySelector('#followup-error').textContent=e.message;}
  ...
  return {show,hide(){active=false;epoch++;sequence++;dialogSequence++;clearInterval(clock);dialog.close();}};
}
