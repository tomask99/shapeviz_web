  ...
  return async function presentationPage(req, res) {
    ...
    try {
      ...
      const headers = {
        ...
        'Content-Security-Policy': csp.replace("script-src 'self'", `script-src 'self' ${new URL(env.SUPABASE_URL).origin}`).replace("style-src 'self'", `style-src 'self' ${new URL(env.SUPABASE_URL).origin}`),
