...
async function readJson(req, maximum = 8192) {
  const chunks = [];
  let size = 0;
...
export function createPresentationEventHandler({ env = process.env, send = fetch, presentationsRoot = path.resolve('presentations') } = {}) {
  ...
  return async function presentationEvent(req, res) {
    ...
    const expectedOrigin = env.SITE_URL || (req.headers.host ? `http://${req.headers.host}` : null);
    ...
    if (!response.ok) { json(res, 502, { ok: false }); return; }
