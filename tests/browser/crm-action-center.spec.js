...
test('Today completes and reschedules in place, refreshes metrics, preserves analytics and mobile layout',async({page})=>{
 ...
 await expect(page.locator('#metrics')).toContainText('9');expect(await root.evaluate(el=>el.compareDocumentPosition(document.querySelector('#business-overview'))&Node.DOCUMENT_POSITION_FOLLOWING)).toBeTruthy();
