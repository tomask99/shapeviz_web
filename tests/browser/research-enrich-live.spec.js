...
test('live Lead enrichment preserves manual CRM values and immutable research while applying selected evidence',async({page})=>{
  ...
  try{
    ...
    const seedCompany=async(account,name)=>(await request('/rest/v1/crm_companies',{method:'POST',jwt:account.jwt,representation:true,body:{owner_id:account.user.id,company_name:name,website:'https://enrichment.example/',country:'SK',industry:'Manually selected industry',short_description:'Keep the manual company description.',fit:'LOW',services:['Web'],pipeline_status:'QUALIFIED',priority:'HIGH',lead_source:'Referral',estimated_value:'12345.67',value_type:'ONE_TIME',instagram:'https://instagram.com/private-fixture-reference'}}))[0];
