...
// A deliberately edited sequence: 3 lifestyle, 3 details, 3 studio, 2 films.
const selection = [
  ['lifestyle_01', 'wide', '50% 66%'], ['detail_01', 'narrow', '50% 55%'],
  ['studio_01', 'narrow'], ['detail_02', 'narrow'], ['studio_03', 'narrow'],
  ['lifestyle_05', 'narrow', '50% 65%'], ['lifestyle_06', 'wide', '50% 64%'],
  ['studio_04', 'wide'], ['detail_04', 'narrow'],
  ['video_creative_02', 'film'], ['video_creative_03', 'film'],
].map(([id, layout, position = '50% 50%']) => {
  const group = groups.find(group => group.items.some(item => item.id === id));
  if (!group) throw new Error('Missing curated gallery asset: ' + id);
  return { ...group.items.find(item => item.id === id), category: group.id, layout, position };
});
const labels = { lifestyle: 'LIFESTYLE', details: 'DETAIL / MATERIAL', studio: 'STUDIO', videos: 'MOTION' };
const markup = `<!-- GALLERY START -->
      <div class="gallery-toggle-wrap"><button id="gallery-toggle" class="gallery-toggle micro" aria-expanded="false" aria-controls="expanded-gallery"><span>EXPAND GALLERY</span>${arrow}</button><span class="gallery-count micro">09 IMAGES / 02 FILMS</span></div>
      <div id="expanded-gallery" hidden>
        ...
        <div class="gallery-toolbar"><span class="micro gallery-edition">SELECTED WORKS <span class="accent">/</span> 01 &mdash; 11</span><button class="gallery-collapse micro" type="button" aria-controls="expanded-gallery">CLOSE ${arrow}</button></div>
        <div class="gallery-grid" aria-label="Selected images and films">${selection.map((item, index) => `<div class="gallery-slot gallery-slot--${item.layout}"><figure class="gallery-item${item.video ? ' gallery-film' : ''}" data-gallery-file="${item.file}" data-kind="${item.category}" style="--image-position:${item.position}">${item.video ? video(item) : image(item)}<figcaption><span>${labels[item.category]}</span><span>${String(index + 1).padStart(2, '0')}</span></figcaption></figure></div>`).join('\n')}</div>
