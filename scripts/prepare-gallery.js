...
const markup = `<!-- GALLERY START -->
      ...
      <div id="expanded-gallery" hidden>
        ...
        ${groups.map((group, index) => `<section class="gallery-section" aria-labelledby="gallery-${group.id}"><header class="gallery-heading" data-reveal><span class="micro accent">0${index + 1} / ${String(group.items.length).padStart(2, '0')} ${index === 3 ? 'FILMS' : 'IMAGES'}</span><h3 id="gallery-${group.id}">${group.title}</h3><p>${group.description}</p></header><div class="gallery-grid">${group.items.map((item, i) => `<figure class="gallery-item${item.video ? ' gallery-film' : ''}" data-reveal data-gallery-file="${item.file}">${item.video ? video(item) : image(item)}<figcaption><span>${group.id === 'videos' ? 'MOTION STUDY' : 'SHAPEVIZ'}</span><span>${String(i + 1).padStart(2, '0')} / ${String(group.items.length).padStart(2, '0')}</span></figcaption></figure>`).join('\n')}</div></section>`).join('\n')}
