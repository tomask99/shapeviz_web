...
**Completed and verified: the 3A MVP, 3B.1–3B.2c, 3C.1 read tools and 3C.2a–3C.2b MCP transport and external authorization. 3C.2c is deployed; the owner's actual local Work conversation discovered Shapeviz and successfully called its catalog tool.** Eight scoped read operations plus import validation support separate opaque OAuth credentials, explicit owner consent and revocation. Production is live at `https://shapevizweb.vercel.app`; web and native OAuth registrations are configured, and the production native OAuth/MCP fixture passes. Access remains bounded to at most one hour and the source session. See [3C.2c](ai-research-phase-3c2c.md) and the [import validation follow-up](ai-research-import-validation.md). Existing UI workflows include explicit enrichment and immutable report history. No paid AI API, model SDK or AI credential is introduced. Fresh conversation first-turn behavior and client disconnect/reconnection remain acceptance checks; a separate web connection has not been created. The rest of Phase 3 is not complete. The Phase 3 checkpoint was pushed as `bb9a539`; the validation follow-up is included in the [25 September completion checkpoint](completion-checkpoint-2026-09-25.md).

## Repository audit and reuse

**Import validation follow-up:** a ninth, non-writing MCP operation, `validate_research_import`, now runs the web import validator on the exact final JSON before delivery. Server and catalog instructions require repair and revalidation. The Phase 3 checkpoint was pushed as `bb9a539c85fffc43929318f78f17a94244510e18`; this follow-up was deployed separately and is included in the [25 September completion checkpoint](completion-checkpoint-2026-09-25.md). See [implementation and verification](ai-research-import-validation.md). This does not implement the reviewed write tools in 3C.3.
