alter table public.presentation_projects
  ...
  add column template_match text,
  add column parent_slug text references public.presentation_projects(deck_slug) on delete set null;
create index presentation_projects_parent_idx on public.presentation_projects(parent_slug);
alter table public.presentation_events add column active_seconds integer not null default 0 check(active_seconds between 0 and 300);
update public.presentation_projects set is_template=true,template_match=client where source_type='template';
...
with sessions as (
  ...
  select session_id,count(distinct slide_index) as slides from events where event_type='slide_viewed' group by session_id
), daily as (
  ...
  select deck_slug,count(*) as visits,sum(active_seconds) as seconds from sessions group by deck_slug
), recent as (
  select s.id,s.deck_slug,s.started_at,s.last_seen_at,s.active_seconds,s.max_slide,s.user_agent_category,
    coalesce(c.slides,0) as slides_viewed
  from sessions s left join counts c on c.session_id=s.id order by s.started_at desc limit 100
)
select jsonb_build_object(
  'summary',(select jsonb_build_object('visits',count(*),'seconds',coalesce(sum(active_seconds),0),'average_seconds',coalesce(round(avg(active_seconds)),0),'slide_views',(select count(*) from events where event_type='slide_viewed')) from sessions),
