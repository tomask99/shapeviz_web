...
do $$ declare c uuid; slug text:=current_setting('crm.test_slug');begin
 ...
 if (select engagement from public.crm_company_signals where id=c)<>'NONE' then raise exception 'Expected NONE';end if;
...
do $$ declare c uuid:=current_setting('crm.test_company')::uuid;r record;begin
 ...
 if public.crm_engagement_level(1,0,0)<>'COLD' or public.crm_engagement_level(3,0,0)<>'ACTIVE' or public.crm_engagement_level(3,180,0)<>'HOT' then raise exception 'Thresholds incorrect';end if;
